...
const DEMO_USERS_KEY = "personalink_users";
const DEMO_SESSION_KEY = "personalink_session";
  ...
  const signIn = useCallback(async (email, password) => {
    ...
    try {
      if (isSupabaseConfigured) {
        ...
        const profileKey = `personalink_profile_${user.id}`;
    ...
    async (partnerEmail) => {
      ...
      try {
        ...
        if (isSupabaseConfigured) {
          ...
          const invites = JSON.parse(localStorage.getItem("personalink_invites") || "[]");
          invites.push({
          ...
          localStorage.setItem("personalink_invites", JSON.stringify(invites));
        }
