...
function scoreLoveLanguageClash(partnerA, partnerB) {
  ...
  const primaryMatch = a.primary === b.primary;
  const secondaryMatch = a.secondary === b.secondary;
  const perfectSwap = a.primary === b.secondary && b.primary === a.secondary;
  const singleCrossMatch = a.primary === b.secondary || b.primary === a.secondary;
  ...
  if (primaryMatch && secondaryMatch) {
    ...
    score = 30;
  } else if (singleCrossMatch) {
    score = 50;
  } else if (secondaryMatch && !primaryMatch) {
    score = 75;
  } else {
    score = 100;
  }
...
function buildLoveLanguageHeadline(clash) {
  ...
  if (score <= 75) {
    ...
    return (
      ...
      'preferences. You already have a bridge — it just needs a little more ' +
      'intentional traffic.'
    );
  }
